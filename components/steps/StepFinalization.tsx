...
const StepFinalization: React.FC = () => {
  ...
  useEffect(() => {
    // Ensure all flags are set correctly upon reaching finalization
    ...
    if(!config.setupFlags.firstLaunchCompleted) {
        ...
        setupFlags: {
            ...prev.setupFlags,
            firstLaunchCompleted: true, // Should be set by wizard on completion
            llmProvidersConfigured: Object.values(prev.llmConfig.providers).some(p => p.isConfigured),
            // dataSourcesConfigured is handled by skip or input in its step
            connectivityValidated: prev.setupFlags.connectivityValidated // This should be true to reach here ideally
        },
        userProfile: {
            ...prev.userProfile,
    ...
    <div className="max-w-lg mx-auto text-center">
      ...
      <div className="bg-slate-50 border border-slate-200 p-6 rounded-lg text-left shadow-sm mb-8">
        ...
        <ul className="space-y-3 text-sm text-slate-600">
          ...
          <li className="flex items-center">
            <CheckIcon className="w-5 h-5 text-green-500 mr-2 flex-shrink-0" />
            Connectivité des services validée.
          </li>
        </ul>
      </div>
      ...
      <p className="text-xs text-slate-500">
        ...
        Vous pouvez modifier ces paramètres plus tard depuis les options de l'application (non implémenté dans cette version).
      </p>
