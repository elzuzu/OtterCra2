...
import { CpuChipIconSolid, PlusCircleIcon } from '../icons'; // Added PlusCircleIcon
...
const StepLlmProviders: React.FC = () => {
  ...
  // Initialize activeInputProviderIds based on current config
  useEffect(() => {
    ...
    (Object.keys(LLM_PROVIDERS_DETAILS) as LlmProviderId[]).forEach(id => {
        ...
        // Show input if a key exists, if it's marked as configured, or if its status indicates a past attempt (error/invalid)
  ...
  // Effect to update activeInputProviderIds if a key is cleared for a non-configured provider
  useEffect(() => {
    setActiveInputProviderIds(currentActiveIds => {
        ...
        currentActiveIds.forEach(id => {
            const providerConf = config.llmConfig.providers[id];
            // If it was active, key is now empty, status is pending (reset), and it was never successfully configured
  ...
  // Effect to check if at least one provider is valid and update global flag
  useEffect(() => {
    ...
    if (isValid !== config.setupFlags.llmProvidersConfigured) { // only update if different
        updateConfig(prev => ({
            ...prev,
    ...
    <div className="max-w-xl mx-auto">
      ...
      <div className="space-y-6">
        {providerOrder.map(providerId => {
          ...
          // Show input if it's active, or if a key exists, or if it's configured, or if status is error/invalid
