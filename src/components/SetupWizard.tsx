  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-slate-100">
      <div className="w-full max-w-3xl bg-white shadow-2xl rounded-xl overflow-hidden">
                ...
                {WIZARD_STEPS_ORDER.map((stepId, index) => (
                    <React.Fragment key={stepId}>
                        <div className="flex flex-col items-center">
                            <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-semibold transition-all duration-300 ${getStepStatus(index)}`}>
                                {index + 1}
                            </div>
                            <p className={`mt-1 text-sm text-center ${index === currentStepIndex ? 'text-blue-600 font-semibold' : 'text-slate-500'}`}>{WIZARD_STEPS_CONFIG[stepId].title.split(' ')[0]}</p>
                        </div>
        ...
        <main className="p-6 md:p-8 min-h-[380px]">
          {renderStepContent()}
        </main>
        ...
        <footer className="bg-slate-50 p-6 flex justify-between items-center border-t border-slate-200">
          ...
          <div className="space-x-3">
            ...
            <button
              ...
              className={`px-6 py-2.5 text-sm font-semibold text-white rounded-md shadow-md transition-all duration-150 ${
                isNextDisabled()
