...
import { ArrowPathIcon, CheckIcon, InformationCircleIcon, ServerStackIconSolid, XCircleIcon, FolderOpenIcon, EnvelopeIcon } from '../icons';
...
const StepTestConnectivity: React.FC = () => {
  ...
  const runAllTests = useCallback(async () => {
    ...
    let allLlmSuccessful = configuredLlmProviders.length > 0; 
    ...
    if (configuredLlmProviders.length > 0) {
        ...
        const llmResults = await Promise.all(llmTestPromises);
         setTestResults(prevResults => [
            ...llmResults,
            ...prevResults.filter(r => r.type === 'dataSource' || r.id === 'llm_placeholder' || r.id === 'dataSource_placeholder') 
        ]);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    JSON.stringify(configuredLlmProviders.map(p => p.id + p.apiKey)), 
    updateConfig,
  ...
  useEffect(() => {
    // Run tests when the component mounts or when key dependencies for tests change.
    runAllTests();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [runAllTests]); 
